...
class AmendMembershipAttestationFlowHandler(
    ...
    private class Handler(private val session: FlowSession) : FlowLogic<SignedTransaction>() {
        ...
        private companion object {
            object HandleAmendMembershipAttestationStep : Step("Handling membership attestation amendment.") {
                override fun childProgressTracker() = tracker()
            }
        }

        override val progressTracker = ProgressTracker(HandleAmendMembershipAttestationStep)

        @Suspendable
        override fun call(): SignedTransaction {
            currentStep(HandleAmendMembershipAttestationStep)
            return subFlow(
                AmendMembershipAttestationFlowHandler(
                    session,
                    progressTracker = HandleAmendMembershipAttestationStep.childProgressTracker()
                )
            )
        }
    }
