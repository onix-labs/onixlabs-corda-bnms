...
class RevokeMembershipAttestationFlowHandler(
    ...
    @InitiatedBy(RevokeMembershipAttestationFlow.Initiator::class)
    private class Handler(private val session: FlowSession) : FlowLogic<SignedTransaction>() {

        private companion object {
            object HandleRevokedMembershipAttestationStep : Step("Handling membership attestation revocation.") {
                override fun childProgressTracker() = tracker()
            }
        }

        override val progressTracker = ProgressTracker(HandleRevokedMembershipAttestationStep)

        @Suspendable
        override fun call(): SignedTransaction {
            currentStep(HandleRevokedMembershipAttestationStep)
            return subFlow(
                RevokeMembershipAttestationFlowHandler(
                    session,
                    progressTracker = HandleRevokedMembershipAttestationStep.childProgressTracker()
                )
            )
        }
    }
