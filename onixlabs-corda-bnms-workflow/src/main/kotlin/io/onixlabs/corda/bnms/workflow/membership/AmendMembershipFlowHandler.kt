...
class AmendMembershipFlowHandler(
    ...
    @InitiatedBy(AmendMembershipFlow.Initiator::class)
    private class Handler(private val session: FlowSession) : FlowLogic<SignedTransaction>() {
        ...
        override val progressTracker = ProgressTracker(HandleAmendedMembershipStep)

        @Suspendable
        override fun call(): SignedTransaction {
            currentStep(HandleAmendedMembershipStep)
            return subFlow(
                AmendMembershipFlowHandler(
                    session,
                    progressTracker = HandleAmendedMembershipStep.childProgressTracker()
                )
            )
        }
    }
