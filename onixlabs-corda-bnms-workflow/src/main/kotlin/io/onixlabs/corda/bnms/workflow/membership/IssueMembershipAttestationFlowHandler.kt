...
class IssueMembershipAttestationFlowHandler(
    ...
    @InitiatedBy(IssueMembershipAttestationFlow.Initiator::class)
    private class Handler(private val session: FlowSession) : FlowLogic<SignedTransaction>() {

        private companion object {
            object HandleIssueMembershipAttestationStep : Step("Handling membership attestation issuance.") {
                override fun childProgressTracker() = tracker()
            }
        }

        override val progressTracker = ProgressTracker(HandleIssueMembershipAttestationStep)

        @Suspendable
        override fun call(): SignedTransaction {
            currentStep(HandleIssueMembershipAttestationStep)
            return subFlow(
                IssueMembershipAttestationFlowHandler(
                    session,
                    progressTracker = HandleIssueMembershipAttestationStep.childProgressTracker()
                )
            )
        }
    }
