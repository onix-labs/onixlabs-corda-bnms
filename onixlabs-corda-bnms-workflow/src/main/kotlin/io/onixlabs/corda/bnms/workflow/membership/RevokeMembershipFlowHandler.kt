...
class RevokeMembershipFlowHandler(
    ...
    @InitiatedBy(RevokeMembershipFlow.Initiator::class)
    private class Handler(private val session: FlowSession) : FlowLogic<SignedTransaction>() {

        private companion object {
            object HandleRevokedMembershipStep : Step("Handling membership revocation.") {
                override fun childProgressTracker() = tracker()
            }
        }

        override val progressTracker = ProgressTracker(HandleRevokedMembershipStep)

        @Suspendable
        override fun call(): SignedTransaction {
            currentStep(HandleRevokedMembershipStep)
            return subFlow(
                RevokeMembershipFlowHandler(
                    session,
                    progressTracker = HandleRevokedMembershipStep.childProgressTracker()
                )
            )
        }
    }
